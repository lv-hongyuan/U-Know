...
Page({
  ...
  onShow() {
    this.applyI18n();
    if (isLoggedIn()) {
      markCategoryRead("like").catch(() => {});
    }
  },

  applyI18n() {
    this.setData({ t: getI18nData() });
    wx.setNavigationBarTitle({ title: t("nav.messageLikes") });
  },

  actionText(notifyType) {
    if (notifyType === "collect") return t("message.collectedYourPost");
    if (notifyType === "comment_like") return t("message.likedYourComment");
    return t("message.likedYourPost");
  },

  mapItem(row) {
    const isCommentLike = row.notifyType === "comment_like";
    return {
      id: row.id,
  ...
  async loadList({ reset }) {
    ...
    try {
      ...
      if (reqId !== this._listReqId) return;
      console.error("load like notifies failed", err);
  ...
  onTapCover(e) {
    ...
    if (notifytype === "comment_like" && commentid && Number(commentmissing) !== 1) {
